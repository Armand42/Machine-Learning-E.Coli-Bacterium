...
# -*- coding: utf-8 -*-
"""
Created on Sun Dec  1 10:59:55 2019
...
def buildSVM(X,y,y2,rocCurveName):
    # y prediction
    newY = dataset[y]
    newY2 = dataset[y2]
    ...
    grid = GridSearchCV(estimator=lasso, scoring = "neg_mean_squared_error",param_grid = dict(alpha=alpha_lasso), cv = 10)
    grid.fit(X,newYY)
    ...
    plt.plot(mean_fpr, mean_tpr, color='b',label=r'Mean Combined ROC (AUC = %0.2f $\pm$)' % (mean_auc),lw=2, alpha=.8)
...
def buildSVM2(X,y,y2, prCurveName):
    # y prediction
    newY = dataset[y]
    newY2 = dataset[y2]
    # Combining medium and stress into an array
...
buildSVM(X,'Medium','Stress','ROC with 10-Fold Cross Validation (Composite SVM)')
buildSVM2(X,'Medium','Stress','Precision Recall (Composite SVM)')
